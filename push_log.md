...
- 【】标签默认不加粗（以参考文件为准）
- 公司介绍内容不加粗
- 教育背景：五号字(10.5pt)、1.5倍行距、无段前距、无开头空格
- 基本信息：字段之间不要空行（姓名性别之间、年龄学历之间等去掉空行）
- 职业经历时长格式：`2008.12-2026.06（17年6个月） 公司 职位`
- 离职原因/看机会原因区分：已离职写"离职原因"，在职写"看机会原因"
- 图片规则：有参考文件时默认使用其模块标题图片（基本信息/教育背景/工作经历）
- 无参考文件时纯文字排版，模块标题用【】文字形式
